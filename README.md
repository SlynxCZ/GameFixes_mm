...
| `voice_flood` | The "server lagger": a client floods `clc_VoiceData` (hundreds of messages a tick, empty voice, thousands of `packet_offsets`) and the server lags copying each one to every listener. In `CServerSideClient::ProcessVoiceData`, a message with more `packet_offsets` than `max_packets` or than it has voice bytes, or a client past `max_per_second` messages, is swallowed; with `kick` the client is kicked. |
