...
| `voice_flood` | The "server lagger": a client floods `clc_VoiceData` (hundreds of messages a tick, empty voice, thousands of `packet_offsets`) and the server lags copying each one to every listener. In `CServerSideClient::ProcessVoiceData`, a message with more `packet_offsets` than `max_packets` or than it has voice bytes, or a client past `max_per_second` messages, is swallowed and the client's voice is dropped for `cooldown` seconds; with `kick`, a client caught `kick_strikes` times (each within a minute of the last) is kicked, so a flooder goes and a real client bursting once doesn't. |
